...
void Bureaucrat::signForm(Form &ff)
{
	try
	{
		ff.beSigned(*this);
		std::cout <<  getName() << " signs " << ff.getName() << std::endl;
	}
	catch (std::exception & e)
	{
		std::cerr <<  getName() << " cannot sign " << ff.getName() << " because " << e.what() << std::endl;
	}
}
...
void Bureaucrat::executeForm(Form const & form)
{
	...
	{
		std::cerr <<  getName() << " cannot executes " << form.getName() << " because " << e.what() << std::endl;
	}
}
